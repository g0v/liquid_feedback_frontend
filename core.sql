...
CREATE TABLE "policy" (
        ...
        "majority_positive"     INT4            NOT NULL DEFAULT 0,
        "majority_non_negative" INT4            NOT NULL DEFAULT 0,
        "majority_indirect"     BOOLEAN         NOT NULL DEFAULT TRUE,
...
COMMENT ON COLUMN "policy"."majority_num"          IS   'Numerator of fraction of majority to be reached during voting by an initiative to be "attainable"';
COMMENT ON COLUMN "policy"."majority_den"          IS 'Denominator of fraction of majority to be reached during voting by an initiative to be "attainable"';
...
COMMENT ON COLUMN "policy"."majority_positive"     IS 'Absolute number of "positive_votes" neccessary for an initiative to be "attainable".';
...
COMMENT ON COLUMN "policy"."no_multistage_majority" IS 'Causes initiatives with "multistage_majority" flag to not be "eligible", thus disallowing them to be winner. See comment on column "initiative"."multistage_majority". This flag does not affect the winner, if "policy"."majority_indirect" is set to TRUE or "policy"."majority_num" is set to zero.';
...
CREATE TABLE "initiative" (
        ...
        "negative_votes"        INT4,
        "attainable"            BOOLEAN,
        "favored_to_status_quo" BOOLEAN,
        "unfavored_to_status_quo" BOOLEAN,
        "schulze_rank"          INT4,
        ...
        CONSTRAINT "non_admitted_initiatives_cant_contain_voting_results" CHECK (
          ...
          ( "positive_votes" ISNULL AND "negative_votes" ISNULL AND "attainable" ISNULL AND
            "favored_to_status_quo" ISNULL AND "unfavored_to_status_quo" ISNULL AND
            "schulze_rank" ISNULL AND
            "multistage_majority" ISNULL AND "reverse_beat_path" ISNULL AND
            "winner" ISNULL ) ),
        CONSTRAINT "favored_excludes_unfavored" CHECK (NOT ("favored_to_status_quo" AND "unfavored_to_status_quo")) );
...
COMMENT ON COLUMN "initiative"."attainable"              IS 'TRUE, if "positive_votes"/("positive_votes"+"negative_votes") is strictly greater or greater-equal than "majority_num"/"majority_den", and "positive_votes" is greater-equal than "majority_positive", and ("positive_votes"+abstentions) is greater-equal than "majority_non_negative"';
COMMENT ON COLUMN "initiative"."favored_to_status_quo"   IS 'TRUE, if initiative has a schulze-ranking better than the status quo (without tie-breaking)';
COMMENT ON COLUMN "initiative"."unfavored_to_status_quo" IS 'TRUE, if initiative has a schulze-ranking worse than the status quo (without tie-breaking)';
...
COMMENT ON COLUMN "initiative"."reverse_beat_path"       IS 'TRUE, if there a beat path (may include ties), from this initiative to the status quo';
COMMENT ON COLUMN "initiative"."eligible"                IS 'Initiative is "attainable" and depending on selected policy has no "multistage_majority" or "reverse_beat_path"';
...
COMMENT ON COLUMN "membership"."autoreject" IS 'TRUE = member votes against all initiatives, if he/she is neither direct_ or delegating_voter; Entries in the "issue_autoreject" table can override this setting.';
...
CREATE TABLE "issue_autoreject" (
        PRIMARY KEY ("issue_id", "member_id"),
        "issue_id"              INT4            REFERENCES "issue" ("id") ON DELETE CASCADE ON UPDATE CASCADE,
        "member_id"             INT4            REFERENCES "member" ("id") ON DELETE CASCADE ON UPDATE CASCADE,
        "autoreject"            BOOLEAN         NOT NULL );
CREATE INDEX "issue_autoreject_member_id_idx" ON "issue_autoreject" ("member_id");

COMMENT ON TABLE "issue_autoreject" IS 'If autoreject=TRUE, then member votes against all initiatives, if he/she is neither direct_ or delegating_voter; Values of either TRUE or FALSE override settings in "membership" table.';
...
COMMENT ON TABLE "direct_population_snapshot" IS 'Snapshot of active members having either a "membership" in the "area", an "interest" in the "issue", or "issue_autoreject"."autoreject" set to TRUE';
  ...
  LANGUAGE 'plpgsql' VOLATILE AS $$
    ...
    BEGIN
      ...
      INSERT INTO "direct_population_snapshot"
        ...
        FROM "issue"
        JOIN "area" ON "issue"."area_id" = "area"."id"
        JOIN "issue_autoreject" ON "issue"."id" = "issue_autoreject"."issue_id"
        JOIN "member" ON "issue_autoreject"."member_id" = "member"."id"
        JOIN "privilege"
          ON "privilege"."unit_id" = "area"."unit_id"
          AND "privilege"."member_id" = "member"."id"
        WHERE "issue"."id" = "issue_id_p"
...
CREATE FUNCTION "close_voting"("issue_id_p" "issue"."id"%TYPE)
  ...
  LANGUAGE 'plpgsql' VOLATILE AS $$
    ...
    BEGIN
      ...
      -- consider delegations and auto-reject:
      ...
      FOR "member_id_v" IN
        SELECT "issue_autoreject"."member_id"
          FROM "issue_autoreject"
          JOIN "member"
            ON "issue_autoreject"."member_id" = "member"."id"
          JOIN "privilege"
            ON "privilege"."unit_id" = "unit_id_v"
            AND "privilege"."member_id" = "member"."id"
          LEFT JOIN "direct_voter"
            ON "issue_autoreject"."member_id" = "direct_voter"."member_id"
            AND "issue_autoreject"."issue_id" = "direct_voter"."issue_id"
          LEFT JOIN "delegating_voter"
            ON "issue_autoreject"."member_id" = "delegating_voter"."member_id"
            AND "issue_autoreject"."issue_id" = "delegating_voter"."issue_id"
          WHERE "issue_autoreject"."issue_id" = "issue_id_p"
          AND "issue_autoreject"."autoreject" = TRUE
          AND "member"."active"
          AND "privilege"."voting_right"
        ...
        SELECT "membership"."member_id"
          ...
          JOIN "privilege"
            ON "privilege"."unit_id" = "unit_id_v"
            AND "privilege"."member_id" = "member"."id"
          LEFT JOIN "issue_autoreject"
            ON "membership"."member_id" = "issue_autoreject"."member_id"
            AND "issue_autoreject"."issue_id" = "issue_id_p"
          LEFT JOIN "direct_voter"
          ...
          AND "member"."active"
          AND "privilege"."voting_right"
          AND "issue_autoreject"."autoreject" ISNULL
          AND "direct_voter"."member_id" ISNULL
          AND "delegating_voter"."member_id" ISNULL
      LOOP
    ...
    BEGIN
      ...
      IF "dimension_v" > 1 THEN
        ...
        LOOP
          UPDATE "initiative" SET
            "attainable" =
              CASE WHEN "policy_row"."majority_strict" THEN
                "positive_votes" * "policy_row"."majority_den" >
                "policy_row"."majority_num" * ("positive_votes"+"negative_votes")
              ELSE
                "positive_votes" * "policy_row"."majority_den" >=
                "policy_row"."majority_num" * ("positive_votes"+"negative_votes")
              END
              AND "positive_votes" >= "policy_row"."majority_positive"
              AND "issue_row"."voter_count"-"negative_votes" >=
                  "policy_row"."majority_non_negative",
            "favored_to_status_quo"   = "rank_ary"["i"] < "rank_ary"["dimension_v"],
            "unfavored_to_status_quo" = "rank_ary"["i"] > "rank_ary"["dimension_v"],
        ...
        END IF;
        if "policy_row"."majority_indirect" THEN
          -- take indirect majorities into account:
          LOOP
            UPDATE "initiative" SET "attainable" = TRUE
              FROM (
                ...
                JOIN "initiative" "new_initiative"
                  ...
                  AND "new_initiative"."attainable" = FALSE
                ...
                AND "old_initiative"."attainable" = TRUE
                AND CASE WHEN "policy_row"."majority_strict" THEN
                  "battle_win"."count" * "policy_row"."majority_den" >
                  "policy_row"."majority_num" *
                ...
                ELSE
                  ...
                  ("battle_win"."count"+"battle_lose"."count")
                END
                AND "battle_win"."count" >= "policy_row"."majority_positive"
